...
In this article, we will be looking at how to set up localization in a Next.js application with the App Router.
...
For the purposes of this post, we will be building out a basic e-commerce application. To see a live working version, check out the [demo app](https://localization-in-nextjs-sample-app.vercel.app/en). You can also access the [complete source code on GitHub](https://github.com/albertdugba/localization-in-nextjs-sample-app).

### Create a new Next.js application

I prefer to use `pnpm` but you can use any package manager of your choice.
...
After the installation, the first step is to create a messages folder at the root of your application. You can save the messages locally or fetch them from a remote source depending on your workflow.
...
The next step is to configure the `createNextIntlPlugin` plugin from the `next-intl` package in your `next.config` file. This plugin will provide the i18n configuration to the server components as follows:
...
### Create `i18n.ts` file to set up the configuration

After this, create an `i18n.ts` file. This is to create a request-scoped configuration that can be used to provide messages based on the user's locale in the server components.
...
It is now time to render the `next-intl` messages based on the user settings in the UI. `next-intl` provides a `useTranslations` hook used to render the messages. The hook takes in a `namespace` or a `key` based on the structure of your `"language.json"` file.

To illustrate, let's integrate the translation capabilities into our product app starting from the HeroSection. In our application we have
...
![(Translation Example)](/blog/localisation-with-nextjs/lang.gif)
...
`next-intl` simplifies language switching in Next.js by automatically handling locale information within standard navigation APIs. By employing shared pathnames, you can directly map Next.js routes to user-requested URLs without additional complexity.

With this configuration, you gain access to routing components and methods like `Link` and `usePathname`, enabling intuitive navigation within your Next.js project. Create a `navigation.ts` in your `src` folder and add the following:
...
I hope this post was helpful to you. You can find the [complete source code on GitHub](https://github.com/albertdugba/localization-in-nextjs-sample-app) and the [demo app](https://localization-in-nextjs-sample-app.vercel.app/en). You can read more on the [official documentation site](https://next-intl-docs.vercel.app/).
